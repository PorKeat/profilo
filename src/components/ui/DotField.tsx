...
const DotField = memo(({
  ...
  sparkle = false,
  waveAmplitude = 0,
  gradientFrom = 'rgba(75, 134, 247, 0.38)',
  gradientTo = 'rgba(139, 92, 246, 0.24)',
  glowColor = '#4B86F7',
  ...rest
}: DotFieldProps) => {
    ...
    function tick() {
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        if (distSq < crSq && eng > 0.01) {
          ...
          if (isBulge) {
            const dotT = 1 - dist / cr;
            const push = dotT * dotT * (p.bulgeStrength as number) * eng;
        ...
        const sparkleOn = p.sparkle && ((((i * 2654435761) ^ (frameCount >> 3)) >>> 0) % 100) < 3;
        const dotRad = sparkleOn ? rad * 1.8 : rad;
